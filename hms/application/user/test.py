...
from hms.domain.user.models import User, UserPersonalData
...
class UserServicesTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.user = self.user_app_services().create_user_from_dict(dict( name=fake.name(),contact_no = fake.phone_number(),   username=fake.first_name(), email=fake.email(), password=make_password(fake.password())))
        return super().setUp()
