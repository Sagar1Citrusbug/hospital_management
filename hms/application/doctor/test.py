...
class DoctorServicesTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.user_data = dict(
          name = fake.name(), contact_no = fake.phone_number(),   username=fake.first_name(), email=fake.email(), password=fake.password()
        )
    ...
    def test_delete_doctor_by_pk(self):
        ...
        pk = self.doctor_create.id
        self.doctor_app_services.delete_doctor_by_pk(pk=pk)
        try:
